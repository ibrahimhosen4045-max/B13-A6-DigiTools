...
const Cards = ({addCard, setAddCard}) => {
  ...
  return (
    <div className="container mx-auto border border-gray-100 mb-10 mt-5 rounded-2xl shadow-lg">
      ...
      {addCard.length === 0 ? (<div className="w-full h-80 flex flex-col gap-2 justify-center items-center text-xl"><img className='w-10' src={cards} alt="" /> Your card is empty!</div>) :
      <div className='px-4 pb-4'>
        {addCard.map(item => 
        <div key={item.id} className="flex items-center justify-between rounded-2xl bg-[#F9FAFC] p-5.5 my-4 transition hover:-translate-y-0.5 hover:shadow-md duration-150">
          <div className="flex items-center gap-3">
            <img className="p-3 border rounded-full border-gray-200 shadow-sm bg-white" src={item.icon} alt=""  onError={(e) => e.target.src = fileImage}/>
            <div>
